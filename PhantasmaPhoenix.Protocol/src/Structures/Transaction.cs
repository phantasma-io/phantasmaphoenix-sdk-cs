...
public sealed class Transaction : ITransaction, ISerializable
{
	...
	public Signature GetTransactionSignature(IKeyPair keypair,
	...
	private void UpdateHash()
	{
		...
		var hash = HashExtensions.Sha256(data);
	...
	public override bool Equals(object obj)
	{
		...
		else if (obj is Transaction tx)
		{
			...
			return this.Hash.Equals(tx.Hash) && this.Payload.SequenceEqual(tx.Payload) &&
				   this.Script.SequenceEqual(tx.Script)
	...
	{
		if (Enum.TryParse(NexusName, ignoreCase: true, out Nexus parsedTxNexus))
		{
			return parsedTxNexus == chainNexus;
		}
		else
		{
			// TODO leaving warning for now, later we should just ignore corrupted txes.
			// Log.Warning("Unknow tx nexus: {nexusName}", NexusName);
		}
