...
public struct Hash : ISerializable, IComparable<Hash>, IEquatable<Hash>
{
	...
	{
		return ByteArrayUtils.ReverseBytes(_data);
	}

	public override string ToString()
	{
		return ByteArrayUtils.ReverseBytes(_data).ToHex();
	}
	...
	public static Hash Parse(string s)
	{
		...
		return new Hash(ByteArrayUtils.ReverseBytes(s.FromHex()));
	}
	...
	public static Hash FromBytes(byte[] input)
	{
		...
		{
			input = HashExtensions.Sha256(input);
		}
	...
	{
		var bytes = HashExtensions.Sha256(str);
		return new Hash(bytes);
	}
